...
import { toastBase, toastVariants } from '@portal/design-core';
...
const containerClasses = computed(() => [
  toastBase,
  toastVariants[props.variant].container,
]);
...
<template>
  ...
  >
    ...
    >
      <!-- Icon -->
      <div :class="['flex-shrink-0', toastVariants[variant].icon]">
        <!-- Info icon -->
      ...
      <div class="flex-1 min-w-0">
        ...
        <button
          v-if="action"
          type="button"
          class="mt-2 text-sm font-medium text-brand-primary hover:text-brand-primaryHover"
      ...
      <button
        v-if="dismissible"
        type="button"
        class="flex-shrink-0 p-1 -m-1 rounded-md text-text-muted hover:text-text-body hover:bg-bg-hover transition-colors"
        aria-label="Dismiss"
        @click="dismiss"
      >
