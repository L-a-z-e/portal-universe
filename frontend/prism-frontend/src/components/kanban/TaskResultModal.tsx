  ...
  useEffect(() => {
    ...
    const loadContext = async () => {
      ...
      try {
        ...
        if (!cancelled) handleError(error, 'Failed to fetch task context');
      } finally {
  ...
  return (
    ...
    >
      <div className="flex flex-col max-h-[calc(100vh-200px)]">
        {/* Scrollable content */}
        <div className="overflow-y-auto flex-1 space-y-6 pr-1 max-h-[60vh]">
          {/* Task Info */}
                  ...
                  <div className="divide-y divide-border-default">
                    {executions.map((exec) => (
                      <div key={exec.id} className="p-4 space-y-3">
                        ...
                        {/* R1: AI Response with Markdown rendering */}
                        {exec.outputResult && (
                          <div>
                            <label className="block text-xs font-medium text-text-heading mb-1">
                              AI Response
                            </label>
                            <div className="bg-bg-input border border-border-default rounded-lg p-3 max-h-60 overflow-y-auto prose prose-sm prose-invert max-w-none">
                              <ReactMarkdown
                                ...
                                components={{
                                  code: ({ children, className }) => {
                                    ...
                                    if (isBlock) {
                                      return (
                                        <pre className="bg-bg-muted rounded p-2 overflow-x-auto">
                                          <code className={className}>{children}</code>
                                        </pre>
                                      );
                                    }
                                    return <code className="bg-bg-muted px-1 rounded text-xs">{children}</code>;
                                  },
                ...
                {context.referencedTasks.map((ref) => (
                  <div key={ref.taskId} className="p-4">
                    ...
                    {ref.lastExecution?.outputResult && (
                      <div className="bg-bg-input border border-border-default rounded-lg p-3 max-h-32 overflow-y-auto prose prose-sm prose-invert max-w-none">
        ...
        {/* R3: Fixed footer - Reject form + Actions */}
