# Portal Universe - 문서 읽기 순서 가이드

> 프로젝트를 빠르게 파악하고, 기존 서비스 개발이나 신규 서비스 추가 시 어떤 문서를 어떤 순서로 읽어야 하는지 안내합니다.

---

## A: 프로젝트 전체 이해

신규 합류 시 가장 먼저 읽을 순서입니다.

### Step 1. 프로젝트 개요

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 1 | [`/README.md`](../../README.md) | 기술 스택, 서비스 포트, 모니터링 |
| 2 | [`/.claude/CLAUDE.md`](../../.claude/CLAUDE.md) | 아키텍처 원칙, 디렉토리 구조, Quick Commands |
| 3 | [`/.claude/rules/common.md`](../../.claude/rules/common.md) | 커밋 컨벤션, 에러 처리 패턴, API 응답 표준 |

### Step 2. 아키텍처 이해

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/docs/architecture/README.md`](../architecture/README.md) | 시스템 설계 문서 목록 |
| 5 | [`/docs/adr/README.md`](../adr/README.md) | 14개 아키텍처 결정 기록 (훑어보기) |
| 6 | [`/docs/diagrams/`](../diagrams/) | 시스템 전체 구조, 서비스 간 통신 다이어그램 |

### Step 3. 환경 구성 & 실행

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 7 | [`/docs/guides/docker-compose.md`](docker-compose.md) | Docker Compose 로컬 개발 환경 구성 |
| 8 | [`/docs/guides/environment-variables.md`](environment-variables.md) | 환경 변수 설정 |

### Step 4. 심화 (선택)

| 문서 | 핵심 내용 |
|------|----------|
| [`/docs/learning/README.md`](../learning/README.md) | PART 0~11 포괄적 학습 자료 |
| [`/docs/scenarios/README.md`](../scenarios/README.md) | 13개 업무 시나리오 |
| [`/docs/prd/`](../prd/) | 제품 요구사항 문서 |

---

## B: 백엔드 서비스 개발

개발할 서비스를 선택하고 해당 경로를 따르세요.

### 공통 (모든 백엔드)

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 1 | [`/.claude/rules/spring.md`](../../.claude/rules/spring.md) | Controller → Service → Repository 패턴, DTO, Validation |
| 2 | [`/services/common-library/docs/README.md`](../../services/common-library/docs/README.md) | ApiResponse, ErrorCode, GlobalExceptionHandler |
| 3 | [`/docs/guides/jwt-rbac-setup.md`](jwt-rbac-setup.md) | JWT 인증, RBAC 권한 설정 |

### B-1. Auth Service

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/services/auth-service/docs/README.md`](../../services/auth-service/docs/README.md) | 서비스 개요, 아키텍처 |
| 5 | [`/services/auth-service/docs/api/auth-api.md`](../../services/auth-service/docs/api/auth-api.md) | 11개 컨트롤러, 38+ API 엔드포인트 |
| 6 | [`/docs/adr/ADR-003-authorization-strategy.md`](../adr/ADR-003-authorization-strategy.md) | 권한 검증 전략 |
| 7 | [`/docs/adr/ADR-004-jwt-rbac-auto-configuration.md`](../adr/ADR-004-jwt-rbac-auto-configuration.md) | JWT 자동 설정 |

### B-2. Shopping Service

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/services/shopping-service/docs/README.md`](../../services/shopping-service/docs/README.md) | 서비스 개요, Saga 패턴 |
| 5 | [`/services/shopping-service/docs/api/product-api.md`](../../services/shopping-service/docs/api/product-api.md) | 상품 CRUD API |
| 6 | [`/docs/scenarios/SCENARIO-002-order-saga.md`](../scenarios/SCENARIO-002-order-saga.md) | 주문/결제 Saga 흐름 |
| 7 | [`/docs/api/shopping-api-reference.md`](../api/shopping-api-reference.md) | 전체 Shopping API 레퍼런스 |

### B-3. Blog Service

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/services/blog-service/docs/README.md`](../../services/blog-service/docs/README.md) | 서비스 개요, MongoDB |
| 5 | [`/services/blog-service/docs/api/blog-api.md`](../../services/blog-service/docs/api/blog-api.md) | Blog API 명세 |
| 6 | [`/docs/scenarios/SCENARIO-008-like-post.md`](../scenarios/SCENARIO-008-like-post.md) | 좋아요 기능 시나리오 |

### B-4. Notification Service

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/services/notification-service/docs/README.md`](../../services/notification-service/docs/README.md) | Kafka 이벤트, WebSocket |
| 5 | [`/services/notification-service/docs/api/notification-api.md`](../../services/notification-service/docs/api/notification-api.md) | 알림 API (6개 엔드포인트) |

### B-5. API Gateway

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/services/api-gateway/docs/README.md`](../../services/api-gateway/docs/README.md) | 라우팅, JWT 검증, Circuit Breaker |
| 5 | [`/docs/guides/security-implementation-spec.md`](security-implementation-spec.md) | Rate Limiting, 감사 로깅 |

---

## C: 프론트엔드 개발

### 공통 (모든 프론트엔드)

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 1 | [`/.claude/rules/typescript.md`](../../.claude/rules/typescript.md) | TypeScript strict mode, 타입 컨벤션, Import 순서 |
| 2 | [`/.claude/rules/tailwind.md`](../../.claude/rules/tailwind.md) | 반응형 패턴, Design Token 3-tier, 다크모드 |
| 3 | [`/frontend/portal-shell/docs/architecture/module-federation.md`](../../frontend/portal-shell/docs/architecture/module-federation.md) | Host-Remote 아키텍처 전체 이해 |

### C-1. Portal Shell (Vue 3 - Host)

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/.claude/rules/vue.md`](../../.claude/rules/vue.md) | Composition API, Pinia, Composable 패턴 |
| 5 | [`/frontend/portal-shell/docs/README.md`](../../frontend/portal-shell/docs/README.md) | Host 개요, 라우팅, 인증 |
| 6 | [`/frontend/portal-shell/docs/api/`](../../frontend/portal-shell/docs/api/) | apiClient, authStore, themeStore 명세 |

### C-2. Blog Frontend (Vue 3 - Remote)

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/.claude/rules/vue.md`](../../.claude/rules/vue.md) | Vue 3 패턴 |
| 5 | [`/frontend/blog-frontend/docs/README.md`](../../frontend/blog-frontend/docs/README.md) | Remote 개요 |
| 6 | [`/frontend/blog-frontend/docs/FEDERATION.md`](../../frontend/blog-frontend/docs/FEDERATION.md) | Module Federation 설정 |
| 7 | [`/frontend/blog-frontend/docs/COMPONENTS.md`](../../frontend/blog-frontend/docs/COMPONENTS.md) | 27개 컴포넌트 레퍼런스 |

### C-3. Shopping Frontend (React 18 - Remote)

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/.claude/rules/react.md`](../../.claude/rules/react.md) | React 컴포넌트, Hooks, Zustand, Module Federation 필수 설정 |
| 5 | [`/frontend/shopping-frontend/docs/README.md`](../../frontend/shopping-frontend/docs/README.md) | Remote 개요 |
| 6 | [`/frontend/shopping-frontend/docs/guides/federation-integration.md`](../../frontend/shopping-frontend/docs/guides/federation-integration.md) | Federation 연동 |
| 7 | [`/frontend/shopping-frontend/docs/learning/`](../../frontend/shopping-frontend/docs/learning/) | React 학습 시리즈 (01~07) |

### C-4. Prism Frontend (React 18 - Remote)

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/.claude/rules/react.md`](../../.claude/rules/react.md) | React 패턴 |
| 5 | [`/frontend/prism-frontend/docs/README.md`](../../frontend/prism-frontend/docs/README.md) | AI 칸반 대시보드 개요 |

### C-5. Design System

| 순서 | 문서 | 핵심 내용 |
|------|------|----------|
| 4 | [`/frontend/design-system-vue/docs/README.md`](../../frontend/design-system-vue/docs/README.md) | Vue 컴포넌트 라이브러리 |
| 5 | [`/frontend/design-system-react/docs/README.md`](../../frontend/design-system-react/docs/README.md) | React 컴포넌트 라이브러리 (30개) |
| 6 | [`/frontend/design-system-vue/docs/COMPONENTS.md`](../../frontend/design-system-vue/docs/COMPONENTS.md) | Vue 컴포넌트 상세 Props/Events |

---

## D: 신규 백엔드 서비스 추가

### Step 1. 설계

| 순서 | 문서 | 왜 읽는가 |
|------|------|----------|
| 1 | [`/docs/adr/README.md`](../adr/README.md) | 기존 아키텍처 결정 검토 |
| 2 | [`/.claude/rules/spring.md`](../../.claude/rules/spring.md) | Layer 구조, 패턴 확인 |
| 3 | 기존 서비스 하나 선택하여 `README.md` + 코드 구조 분석 | 디렉토리 레이아웃 참고 |

### Step 2. 구현

```
services/new-service/
├── src/main/java/com/portal/universe/newservice/
│   ├── controller/           # REST 엔드포인트
│   │   └── dto/              # Request/Response DTO
│   ├── service/              # 비즈니스 로직
│   │   └── impl/
│   ├── repository/           # 데이터 접근
│   ├── entity/               # JPA 엔티티
│   ├── config/               # 서비스 설정
│   └── exception/            # 서비스 에러 코드
├── src/main/resources/
│   └── application.yml       # 프로필별 설정
├── docs/
│   ├── README.md
│   ├── api/                  # API 명세
│   └── architecture/         # 아키텍처 문서
├── build.gradle
└── Dockerfile
```

### Step 3. 인프라 연결

| 순서 | 작업 | 참고 문서 |
|------|------|----------|
| 4 | `docker-compose.yml`에 서비스 추가 | [`/docs/guides/docker-compose.md`](docker-compose.md) |
| 5 | API Gateway에 라우트 추가 | [`/services/api-gateway/docs/README.md`](../../services/api-gateway/docs/README.md) |
| 6 | Kafka 이벤트 연동 (필요시) | [`/docs/learning/README.md`](../learning/README.md) PART 1 |
| 7 | K8s Deployment 작성 (필요시) | [`/docs/guides/kubernetes.md`](kubernetes.md) |

### Step 4. 문서 작성

| 순서 | 작업 | 참고 |
|------|------|------|
| 8 | `docs/README.md` 작성 | 기존 서비스 README 형식 참고 |
| 9 | `docs/api/*.md` 작성 | [`docs_template/guide/api/how-to-write.md`](../../docs_template/guide/api/how-to-write.md) |
| 10 | ADR 작성 (주요 결정이 있었다면) | [`docs_template/guide/adr/how-to-write.md`](../../docs_template/guide/adr/how-to-write.md) |

---

## E: 신규 프론트엔드 Remote 추가

### Step 1. Module Federation 이해

| 순서 | 문서 | 왜 읽는가 |
|------|------|----------|
| 1 | [`/frontend/portal-shell/docs/architecture/module-federation.md`](../../frontend/portal-shell/docs/architecture/module-federation.md) | Host 설정, exposes, remotes 구조 |
| 2 | [`/.claude/rules/react.md`](../../.claude/rules/react.md) 또는 [`vue.md`](../../.claude/rules/vue.md) | Module Federation 체크리스트 (특히 React `react-dom/client` 필수) |

### Step 2. 구현

**React Remote 기준:**
```typescript
// vite.config.ts 필수 설정
federation({
  name: 'new-remote',
  filename: 'remoteEntry.js',
  exposes: {
    './bootstrap': './src/bootstrap.tsx',
  },
  shared: [
    'react',
    'react-dom',
    'react-dom/client',  // 필수! 누락 시 Error #321
  ],
})
```

### Step 3. Host 등록

| 순서 | 작업 | 파일 |
|------|------|------|
| 3 | Portal Shell의 `vite.config.ts` remotes에 추가 | `frontend/portal-shell/vite.config.ts` |
| 4 | `.env` 파일에 Remote URL 환경변수 추가 | `.env.local`, `.env.docker` |
| 5 | Portal Shell 라우트에 경로 추가 | `frontend/portal-shell/src/router/` |
| 6 | Sidebar에 메뉴 항목 추가 | `frontend/portal-shell/src/components/Sidebar.vue` |

### Step 4. 검증

- [ ] `shared`에 `react`, `react-dom`, `react-dom/client` 포함 (React)
- [ ] 빌드 후 `importShared()` 호출 확인
- [ ] Host에서 Remote 로드 테스트
- [ ] Standalone 모드 테스트 (`/standalone` 경로)
- [ ] 다크모드 동기화 확인

---

## 전체 문서 맵 Quick Reference

```
README.md .......................... 프로젝트 개요
.claude/CLAUDE.md .................. 아키텍처 원칙, 포트
.claude/rules/ ..................... 개발 규칙 (Spring, Vue, React, TS, Tailwind)

docs/
├── guides/ ........................ 실행 가이드 (Docker, K8s, 환경변수, 보안)
├── architecture/ .................. 시스템 설계 문서
├── adr/ ........................... 14개 아키텍처 결정 기록
├── scenarios/ ..................... 13개 업무 시나리오
├── learning/ ...................... PART 0~11 학습 자료
├── api/ ........................... API 레퍼런스
├── prd/ ........................... 제품 요구사항
├── diagrams/ ...................... 시스템 다이어그램
├── runbooks/ ...................... 운영 절차서
├── troubleshooting/ ............... 문제 해결 기록
└── testing/ ....................... 테스트 전략

services/*/docs/ ................... 서비스별 문서 (README, API, Architecture)
frontend/*/docs/ ................... 프론트엔드별 문서 (README, Components, Federation)
```
